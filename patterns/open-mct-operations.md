...
- **起動時の無害なコンソールエラー——ただし`router.setPath()`を絡めると無害でなくなる**：
  ...
  **訂正・深掘り(2026-09-16、kikimimiより)**：ただし「常に無害」ではない——
  ...
  呼んでいない)。〔do-survey、2026-09-24〕`tabularmaps/do`自身のダッシュボード
  (Open MCT 4.3.1)でも同じエラーが確認された——doの本体自体もこの無害エラーから
  逃れられていない、という4件目の確認。
...
**ツリーが仮想化(virtualized)されていると、深い階層のリーフはDOMに存在しないことがある。** ブラウザ自動化でDOMを検査しても、スクロールして表示範囲に入れない限りリーフが見つからない。確認・検証時は、URLで直接該当オブジェクトへ遷移する方が確実(`#/browse/<namespace>:<key>/...`の形。`key`にコロンが含まれる場合もそのまま使える)。〔do-survey、2026-09-24〕

**歪んだ(スキューした)分布の色分けには、連続的なランプよりも順序尺度(ordinal)+固定の区切りが読みやすいことがある。** 件数分布が大きく偏っている場合、`scale.type`を`'ordinal'`にし、対象期間の長さ(単年/元号8年分/全期間)ごとに固定の閾値で区切ると、線形ランプより読みやすい。厳密な値は注記(notes)側に出す、という役割分担にする。〔do-survey、2026-09-24〕patterns/interoperability.mdの`scale.colors`注入点(部品を組み込む側が配色を上書きする仕組み)と組み合わせて使える。
