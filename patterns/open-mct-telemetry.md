...
- **Display Layout（複数の計器を1画面に合成する組み込み機能）も、実績ベースでは避けられている**：2026-09-16、kikimimiがm3xx-fleet・sas0のコードを実際に読んで確認した——「合成ダッシュボードはDisplay Layoutで組む」という直感的な設計を、両プロジェクトとも**採用していない**。代わりに「単一の非永続object provider＋単一のview provider」で、`view().show(element)`内にHTMLテーブル・SVG・stat card等を直接組み立てて流し込む、という実装で統合画面を実現している。PlanLayoutと同根の「非永続provider objectとDisplay Layout系の組み込み機能は相性が悪いのでは」という懸念を、そもそも踏まない回避策になっている。kikimimi自身もこれに倣い、Display Layoutを使わず単一カスタムビューで実装した（2026-09-16）。**複数の要素を1画面に合成したい場合、Display Layoutを検討する前に「単一カスタムビュー内で直接DOM生成する」実績のある経路をまず検討する価値がある。**
