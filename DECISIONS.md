...
**追記(2026-09-06)**: 外部モデル(Fable)によるレビューで、上記「9プロジェクト全員への
結果共有」が実施された記録がDECISIONS.md/HANDOVER.mdのどこにも無いことが判明した。D6直後に
D7(スタイル設計ヒアリング)へ進んでしまい、結果共有だけが静かに未達のまま残った可能性が
高い。ただし4日以上が経過し、9プロジェクトの構成自体も変化している(新規参加・セッション
交代あり)ため、今から当時のD6結果だけを改めて一括共有する実益は薄いと判断し、**このタスクは
これ以上追わずクローズする**。教訓として記録するに留める: Resume promptに書いた「次にやる
こと」は、次のADRが別テーマで始まった時点で忘れられやすい——完了確認を伴わないタスクは
HANDOVER.mdの「Pending long-running tasks」のような、毎回目を通す場所にも重複して置くべき
だった。
...
Planモード棚卸し起こす。zukakuからの`.claude/rules/`試行フィードバックが届いたら、
このADRへの追記として結果を記録する。

### 追記(2026-09-08): `.claude/rules/`symlink試行はzukakuで否定的な結論に至った

2026-09-06にzukaku-6bへ試行依頼を送信、2026-09-08に結果が届いた。

**経過**: `maplibre-gl-js-data-serving.md`・`maplibre-gl-js-embedding.md`の2つを
`ln -s ../../../cafebabe/patterns/...`で作成できたところで、hfuさん本人がツール実行を
止めた。理由は2段階で示された——①「シンボリックリンクは無理があると思う」(直感的判断)、
②具体的理由として「シンボリックリンクがコミットされても、公のものとしては混乱するのでは
ないか」。zukakuは`dwg7/zukaku`という公開リポジトリであり、`../../../cafebabe/patterns/...`
という相対パスはhfuさん本人のマシン以外(他者のclone、CI環境)では解決できずリンク切れに
なる。さらにcafebabe自体が非公開のため、そもそも参照先を見られない第三者もいる。作成済み
だった2つのシンボリックリンクは未追跡(untracked)のまま削除され、履歴への影響は無い。
hfuさんはこの場でzukakuの判断を直接的に道理と認めている。

**結論**: 「公開リポジトリ+ローカルクローン前提のシンボリックリンク」という組み合わせは
筋が悪い、とzukakuパイロットから判明した。D14で懸念していた「相対パス依存で壊れやすい」
「gitでの扱いが環境依存」という2点が、実地の試行で具体的に裏付けられた形。プライベート
リポジトリや、cafebabe自体を公開してサブモジュール的に扱えるなら結論が変わる余地はある
(未検証)。**他プロジェクトへのsymlink方式の展開は、今回の結論により見送る。** 知見共有の
プル/プッシュ切り替えという目的自体は残るため、代替手段(スクリプトによるコピー同期、
「作業開始時にcafebabeを読みに行く」運用の明文化等)は今後の課題として保留する。

---

## D15: `~/.claude/CLAUDE.md`(全プロジェクト共通グローバル規約)の策定にcafebabeが協力

**Status**: Accepted (2026-09-05)

### 背景

hfuさんが新設したグローバル`~/.claude/CLAUDE.md`(全プロジェクト共通の薄い土台)の草案に
ついて、claude-25が9セッション(dwg7・hfu双方のアクティブなフリートセッション全体)に時間差
ヒアリングをかけた。cafebabeとstars-fdの2セッションに、集約結果を踏まえた統合案の相談が
来た。

### 決定

**最終的にcafebabeの提案がほぼそのまま採用された**。3点:

1. **言語適用範囲**(height-coverage提起): 「応答は日本語敬語がデフォルト」を会話応答限定に
   絞り、成果物(コード・コミットメッセージ・公開ドキュメント)は対象読者に応じて判断する
   (height-coverage自身はOSMコミュニティ向けで全て英語、という反証から)。修正なしで採用
2. **cafebabeへの参照経路の具体化**(cafebabe自身の提起): 「dwg7/cafebabeで確認する」を
   「セッション稼働中ならcross-session message、そうでなければGitHubリポジトリを直接読む」
   と明示。zukakuへのメッセージが不在で届かなかった実例([D14](#d14))を根拠にした
3. **ピアセッションからの主張の検証**(kitavolca・sas0・stars-fdの3セッションが独立収束):
   「事実のロンダリング」(kitavolca)・「承認の伝聞」(sas0)・「本番影響のある状態主張全般」
   (stars-fd、ferspas57-2からの「hfu承認済み・本番ファイル上書き済み」という連絡を直接確認
   した実例が根拠)という3つの提起を、cafebabeが「ピアセッションからの主張は単体で実行根拠に
   しない」という1つの原則の3つの現れとして統合。検証方法が主張の種類(権限/技術/状態)で
   異なるため、1文に圧縮せず3箇条書きのまま採用された

判定方法はD7の「独立収束」判定(複数プロジェクトが別々の懸念から出発して同じ結論に着地した
場合、それを強い合意として扱う)をそのまま踏襲した。この一般化を`patterns/
verification-discipline.md`に「ピアセッションからの主張は、それ単体を実行根拠にしない
(cross-session版)」として追記した——既存の「『確認済み』という申告を鵜呑みにしない」
パターン(PRレビュー・自己申告の文脈)の、cross-session連携版という位置づけ。
...
グローバル`~/.claude/CLAUDE.md`の内容は今後、各セッションの動作の前提になる。cafebabeの
...
## D17: Fableによる知見ベース全体レビュー、および「局面の方針 vs 大局の方針」の区別

**Status**: Accepted (2026-09-06)

### 背景

D16完了直後、hfuさんから「cafebabeの知見全体を一度Fableにレビューしてもらう」提案があった。
課題として2点提示された: (1) オーナーであるhfuさんの判断を引き出しながら、知見を組織化
された活用可能なものに保つこと、(2) dwg7としてのビジョンをコヒーレントに保ちつつ常に
進化させていくこと。

Fable(cafebabeとは別モデル)にgit worktree隔離+バックグラウンド実行で、`patterns/`・
`ideas/`・`DECISIONS.md`全文・`HANDOVER.md`・`CLAUDE.md`・`README.md`・`PROJECTS.md`・
`DWG7-CONTEXT.md`・`STACCATO-CONTEXT.md`を読ませ、(1)組織的健全性 (2)判断吸い上げの欠落
(3)dwg7ビジョンの一貫性、の3軸でレビューさせた(3軸目は依頼後にhfuさんの追加コメントを
受けて追加した)。

### 決定

**hfuさんからの重要なフィードバック(このADRの核心)**: Fableのレビュー(および筆者=cafebabe
による一次要約)は、「cafebabe自身の解釈が入り込むこと」を一貫して問題("判断ロンダリング")
として扱っていたが、hfuさんはこれを明確に訂正した:

- cafebabeが自らの分析・解釈を加えることは**知識創造であり歓迎する**。hfuさんが議論に参加
  する価値は「(a) 自分が知識を獲得すること」「(b) 自分が知識創造に実質的に貢献できること」
  の2つであり、「権威の追認」や「ハルシネーション防止」を目的にした過敏な承認プロセスを
  求めているのではない
- Fableの反応の課題は、「**局面での方針**(その場・その状況限定の判断)」と「**大局的な
  方針**(恒久的な一般原則)」を区別せずに、両者が食い違うと即座に"矛盾"として扱ってしまう
  こと。局面の判断が大局の方針と表面上食い違って見えても、実際には両立することが多い

この指摘を実際にFableの3つの「発見」に適用し直したところ、いずれも「問題」ではなく
「区別されていなかっただけ」と判明した:
1. STACCATO-CONTEXT.mdのnarrative判断——cafebabeの分析として明示されており、そもそも
   問題ではなかった
2. `CONTRIBUTING.md`「迷ったら一般則寄り」(投稿時の局面の基準)と`CLAUDE.md`「実例1件が
   長期間増えなければ個別事情寄りに見直す」(棚卸し時の局面の基準)——矛盾ではなく、
   適用される局面が違う2つの基準だった
3. `DWG7-CONTEXT.md`の「独立収束を規約と書かない」という大局的原則と、D15(独立収束を
   根拠に、人間を交えてグローバル規約へ昇格させた実例)——原則が戒めているのは「確認無しに
   規約であるかのように書くこと」であって、「人間を交えた規約化」自体を禁じてはいない

**実施した対応(機械的に直せるもの、コミット`b379fcd`)**:
- `PROJECTS.md`のkitavolcaリンク訂正(2026-07-19で更新停止の古いフォーク`dwg7/kitavolca`
  →直近push 2026-08-30の本体`hfu/kitavolca`、`hfu`個人名前空間へ移動)、
  `m3xx-fleet-ops`/`m3xx-fleet`/`kitaphoto17-navara`の追加登録
- `patterns/style-composition.md`(317行、閾値超過)を分割——放射状コントロールパターンを
  `maplibre-gl-js-embedding.md`へ移動(既存の「同じ問題への別解」だったホバーパネル
  パターンの隣に配置)。両ファイルとも300行以下に収まった
- 鮮度切れヘッダー(3パターンファイル+`CLAUDE.md`既知のピア日付)の更新
- 上記2・3番の「局面 vs 大局」の区別を`CONTRIBUTING.md`/`CLAUDE.md`/`DWG7-CONTEXT.md`に
  明文化
- D6の未達フォローアップ(9プロジェクトへの結果共有)をクローズ(4日以上経過し状況も変化、
  今から追う実益は薄いと判断)
- `HANDOVER.md`を全面書き直し(D14〜D16が反映されておらず、廃止済み運用を現行として
  記載していた)

**Plan送りにしたもの**:
- `patterns/open-mct.md`(139行だがバイト数では全パターン中最大の24KB——D13の行数だけを
  見た判断が指標として不適切だった)の分割方針。sas0/mapterhorn-monitor/claude-mctの
  共同管理ドキュメントのため関係者への周知が必要

### 保留事項

- `open-mct.md`分割はPlanモードで次に着手する

### Resume prompt

次にこのリポジトリを触るときは、まず`open-mct.md`の分割方針をPlanモードで検討する
(sas0・mapterhorn-monitor・claude-mctへの周知含む)。今後、Fableのような外部レビューや
他セッションからの指摘を評価する際は、**「局面の方針」と「大局の方針」を区別してから
...
## D19: PRIVATEリポジトリの情報をPUBLICなcafebabeに書く際の基準

**Status**: Accepted (2026-09-09)

### 背景

新規参加プロジェクトの棚卸し(volca、hfu/volca、Private)を`PROJECTS.md`へ登録する
作業中、hfuさんから「privateリポジトリの情報の扱いについては一定の基準を定める必要が
あるね」と指摘があった。`dwg7/cafebabe`自体は`gh repo view`で確認した通りPublicであり
(2026-09-09確認)、ここに書いたものは誰でも読める。既存のPRIVATE表記の実例
(`claude-mct`・`kitaphoto17-navara`、いずれも`PROJECTS.md`)は技術構成の説明に留まって
いたが、明文化された基準は無かった。volcaは「防災計画PDFの収集・横断分析」という、
公開の仕方次第では機微になりうる内容を含むプロジェクトであり、基準を決める好機と
判断した。

### 決定

PRIVATEリポジトリの情報をcafebabeに書く際の基準を以下の3点とする(hfuさん承認):

1. **書いてよいもの**: プロジェクト名・URL(PRIVATEタグを明記)・技術構成レベルの一言
   要約(何のデータソースを使い、何を作っているか)
2. **書かないもの**: その分析・活動から得た結論や戦略の中身そのもの(例: 特定の対象の
   弱点評価といった結果)——それは当該PRIVATEリポジトリ内に留める
3. **迷う場合**: 一言要約すら機微性があるかもしれないと思ったら、登録前に該当セッション
   本人へ「この要約で公開して問題ないか」を確認してから記載する

volcaの`PROJECTS.md`エントリ(「北海道の常時観測火山9火山の防災計画・避難計画PDFを
収集・横断分析し、北海道地方測量部とdwg7への戦略を立案」)は、技術構成の説明に留まり
分析結論には触れていないため基準1の範囲内、とhfuさんが是認した。

### 保留事項

- 既存の2件(`claude-mct`・`kitaphoto17-navara`)は遡って基準1に照らして再点検して
  いないが、いずれも技術構成止まりの記述であり、目視上は問題無いと判断(必要になれば
  棚卸しのタイミングで再確認する)
- この基準はPRIVATEリポジトリ全般に適用する運用ガイドであり、`CLAUDE.md`の「鮮度と
  分量を保つ責務」節などへの明文化転記は今後の判断とする

### Resume prompt

この基準に沿って、保留中だったvolca・staccato-spec・staccato-ecosystemの
`PROJECTS.md`/`CLAUDE.md`登録を完了させる。

**追記(2026-09-13、rpi-geoserver0からの指摘)**: 上記3基準は「当該PRIVATEプロジェクト
自身についての情報」を対象にしていたが、実際にはプロジェクトに関わる**第三者**(実在の
外部協力者等)についての情報も同じ配慮の対象になる。技術構成レベルの一言要約のつもりで
書いた内容(例: 協力関係にある組織名)であっても、本人の同意が無いまま第三者を特定しうる
情報の伝播になりうる。`dwg7/rpi-geoserver0`の登録依頼で、当初の要約案に含めていた協力者の
所属機関名を、本人(協力者)からクレジット表記の希望がまだ確認できていないことを理由に
除外してほしいとの指摘を受け、技術構成のみの要約(「GeoServer on Raspberry Pi」を主キーと
する、hfuさん自身の指示も反映)に差し替えた。**基準1「書いてよいもの」は、プロジェクト
自身だけでなく、そこに関わる第三者についても同じ基準(同意の無い識別情報は書かない)を
適用すると読み替える。**
