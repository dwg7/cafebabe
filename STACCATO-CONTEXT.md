...
## 追記(2026-09-05): ferspas57の「narrative」提案とLibraryスコープの拡張

`ferspas57`(2026-09-03新規参加、9プロジェクト横断議論のサーベイ後に加わったため上記の
表には含まれていない)が実際にend-to-endで稼働し、`NARRATIVE-FORMAT.md`/`NARRATIVES.md`
という具体的な成果物を出した。`staccato-ecosystem`から「narrativeをLibraryレベルの
成果物タイプとして、カタログメタデータ・スタイル(ADR 0007)と並列に位置づけるべきか」
という問いを受け、リポジトリを直接読んで検証した。

**確認できた事実**: `NARRATIVES.md`は「あらかじめ検証済みの、複数ステップからなる
案内(narrative)」を1件収録する事前著述ライブラリで、Staffは新規生成せず**選ぶだけ**
(「不確実なら生成せず『無い』と正直に言う」という`STAFF-PROMPT.md`のAnti-Fabrication
規律の延長)。ただし選んだ後の**言語・レジスタの実行時適応**(例: フランス語で、平易な
表現で)はStaffが都度ライブに行う——つまり「内容(facts)は凍結、提示のされ方は生きている」
という二層構造。

**評価**: 単純に上記1(User→Staffが開発時に前倒しされるという一般化仮説)の実例として
片付けるのは正確ではない——ferspas57は意図の構造化自体はリクエストのたびにライブに行って
いるため、"完全に静的キュレーション型"側には分類できない。むしろ**6番目の洞察
(kaga0が提案した「Live Library」対「Frozen/Cached Library」という区別)の、これまでで
最も具体的な実例**だと考えた方が精度が高い。narrative libraryは「検証済み・バージョン管理
された内容はFrozen、それをどう見せるかの選択と言い回しはLive」という、Live/Frozen軸の
**内部にさらに層がある**ことを示す新しいデータ点になる。

「narrativeをLibraryの成果物タイプとして、カタログ・スタイルと並列に置く」という提案自体は
妥当だと判断した。根拠は`patterns/interoperability.md`の「APIを作るな、単にバージョン管理
された読み取り専用JSONを配信しろ」(sas0 D61-D64由来)という、staccato-specとは独立に
dwg7内で収束していた原則と、構造が同型であること——`stars.optgeo.org`がタイルカタログ
だけでなくスタイル配信(ADR 0007)も「後から気づいたらLibraryの仕事だった」という経緯を
辿ったのと同じ形で、narrativeも「後から気づいたらLibraryの仕事だった」という3件目の実例に
なっている。

ただし1点、区別を残すべきだと考える: カタログ・スタイルは**どのCartographer/Map Intentの
組み合わせにも使い回せる汎用リソース**だが、narrativeは**特定のデータセット・特定の問いに
対して検証済みの、再利用性の低いコンテンツ**(`NARRATIVES.md`自身が「1エントリ」なのも
その性質の表れ)。「Libraryの成果物タイプ」という大枠には収まるが、カタログ・スタイルと
同列の「何にでも使える資源」ではなく、「Staffの検証済み回答の在庫」という別のサブタイプ
として扱う方が、後から見た人が実態を誤解しにくいと考える。

**実例(Known uses、更新)**
- `ferspas57` — `NARRATIVES.md`(1件、英語のみ)/`NARRATIVE-FORMAT.md`(v1、2026-09-05)。
  内容(facts)は凍結・言語/レジスタ適応はStaffがライブに行う、という二層構造を実装

（このドキュメント自体の鮮度について: 2026-09-02の9プロジェクト横断議論の後に発生した
新しい実例をここに追記した。今後も新しい実装が出た際は、この形式(追記+日付)で積み増す。）
