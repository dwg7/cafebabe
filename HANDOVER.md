...
- **tabularmaps/doからの第2弾寄稿(7点+既存パターンの訂正1件)+verification-
  ...
  179にする方針変更(政府資料の根拠つき)。(2) 新規「行政・領土的地位への言及は政府公表
  資料の言い回しのみを使う」(北方領土6村の表記統一、sas0も踏襲)。(3) 気象庁bosai旧URLの
  静かな更新停止+r8 JSON配列の全走査必要性——**sas0で実際に警報取りこぼしのバグが発覚
  (D76)**、独立実装の統合が暗黙の検査になった実例として記録。(4) カルトグラム
  レイアウトへの「オプション要素は外縁列に置く」追加。(5) 部品の他ダッシュボード採択
  (取得/配色/置き場所の3注入点、vendoring+ハッシュ記録)をpatterns/interoperability.md
  に新規。(6) Open MCT内の浮きパネル用CSS色トークンの訂正+GitHub Pages 10分キャッシュ。
  (7) フリート運用でのポート衝突→別プロジェクトのサーバー誤認、をpatterns/
  local-dev-pitfalls.mdに新規。sas0側の主張(D76/D77)はローカルクローンで直接確認して
  裏取りした。あわせて`patterns/verification-discipline.md`(334行、閾値超過)を
  self-checks/cross-session の2ファイルに分割、既存の相互参照リンクも新ファイルに
  更新した
